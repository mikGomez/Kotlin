...
object Conexion {
    ...
    fun addPersona(contexto: AppCompatActivity, p: Simpson):Long{
    ...
    fun obtenerPersonas(contexto: AppCompatActivity):ArrayList<Simpson>{
        var sim:ArrayList<Simpson> = ArrayList(1)

        val admin = AdminSQLIteConexion(contexto, this.DATABASE_NAME, null, DATABASE_VERSION)
        val bd = admin.readableDatabase
        val fila = bd.rawQuery("select nombre,tipo,imagen from simpsons", null)
        while (fila.moveToNext()) {
            var p:Simpson = Simpson(fila.getString(0),fila.getString(1),fila.getString(2))
            sim.add(p)
        }
        bd.close()
        return sim //este arrayList lo puedo poner en un adapter de un RecyclerView por ejemplo.
    }
